...
def test_get_all_admins(admin_service, mock_admin_repo):
    ...
    result = admin_service.get_all_admins(current_user, page=1, limit=10)

    assert len(result["admins"]) == 2
    mock_admin_repo.get_all_admins.assert_called_once_with(1, 10)
...
def test_create_admin_duplicate_email(admin_service, mock_admin_repo):
    ...
    mock_admin_repo.get_admin_by_email.return_value = Admin(
        id=1, username="test1", email="new@example.com", role=RoleEnum.admin, hashed_password=""
    )
...
def test_update_admin(admin_service, mock_admin_repo):
    ...
    mock_admin_repo.get_admin_by_id.return_value = Admin(
        id=1, username="test1", email="test1@example.com", role=RoleEnum.admin, hashed_password=""
    )
    mock_admin_repo.update_admin.return_value = Admin(
        id=1,
        username="updated",
        email="test1@example.com",
        role=RoleEnum.admin,
...
def test_delete_admin(admin_service, mock_admin_repo):
    ...
    mock_admin_repo.get_admin_by_id.return_value = Admin(
        id=2, username="test2", email="test2@example.com", role=RoleEnum.operator, hashed_password=""
    )
